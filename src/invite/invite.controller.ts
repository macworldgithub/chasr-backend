...
export class InviteController {
  ...
  async createInvite(
    @Req() req: any,
    @Body() body: { provider: string; inviteeEmail?: string },
  ) {
    if (!['xero', 'myob', 'quickbooks', 'csv'].includes(body.provider))
  ...
  async handleRedeemXeroCallback(
  ...
  async getRedeemMyobAuthUrl(@Param('token') token: string, @Ip() ip: string) {
    const invite = await this.inviteService.validateAndConsumeInvite(token, ip);
    return this.myobOAuth.generateAuthUrl(
      invite.orgId.toString(),
      `invite:${token.slice(0, 8)}`,
    );
  }

  @Post('redeem/:token/quickbooks/auth-url')
  async getRedeemQuickBooksAuthUrl(
    @Param('token') token: string,
    @Ip() ip: string,
  ) {
    const invite = await this.inviteService.validateAndConsumeInvite(token, ip);
    return this.quickBooksOAuth.generateAuthUrl(
      invite.orgId.toString(),
      `invite:${token.slice(0, 8)}`,
    );
  }
